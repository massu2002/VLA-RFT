...
for _kv in "$@"; do
  case "${_kv}" in
    ...
    NEGATIVE_EVAL_TYPES=*) NEGATIVE_EVAL_TYPES="${_kv#NEGATIVE_EVAL_TYPES=}" ;;
    TEMPORAL_SHIFT_MAX=*) TEMPORAL_SHIFT_MAX="${_kv#TEMPORAL_SHIFT_MAX=}" ;;
    ACTION_NOISE_STD=*) ACTION_NOISE_STD="${_kv#ACTION_NOISE_STD=}" ;;
...
print_run_summary \
  ...
  "NEGATIVE_TYPES"     "${NEGATIVE_EVAL_TYPES}" \
  "TEMP_SHIFT_MAX"     "${TEMPORAL_SHIFT_MAX}" \
