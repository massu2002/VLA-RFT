...
def get_proprio_projector(
    ...
    checkpoint_source = str(checkpoint_path or cfg.pretrained_checkpoint)
    if model_is_on_hf_hub(checkpoint_source):
        ...
        if checkpoint_source not in model_path_to_proprio_projector_name.keys():
        ...
        proprio_projector_path = hf_hub_download(
            repo_id=checkpoint_source, filename=model_path_to_proprio_projector_name[checkpoint_source]
        )
        state_dict = load_component_state_dict(proprio_projector_path)
        proprio_projector.load_state_dict(state_dict)
    else:
        checkpoint_path = find_checkpoint_file(checkpoint_source, "proprio_projector")
...
def get_noisy_action_projector(
    cfg: Any,
    llm_dim: int,
    checkpoint_path: Optional[Union[str, Path]] = None,
    ...
    checkpoint_source = str(checkpoint_path or cfg.pretrained_checkpoint)
    checkpoint_path = find_checkpoint_file(checkpoint_source, "noisy_action_projector")
    state_dict = load_component_state_dict(checkpoint_path)
...
def get_action_head(
    ...
    if model_is_on_hf_hub(checkpoint_source):
        ...
        if checkpoint_source not in model_path_to_action_head_name.keys():
            raise ValueError("Unsupported HF Hub pretrained checkpoint found!")
        # Download proprio projector directly from HF Hub
        action_head_path = hf_hub_download(
            repo_id=checkpoint_source, filename=model_path_to_action_head_name[checkpoint_source]
        )
        state_dict = load_component_state_dict(action_head_path)
        action_head.load_state_dict(state_dict)
    else:
        checkpoint_path = find_checkpoint_file(checkpoint_source, "action_head")
        state_dict = load_component_state_dict(checkpoint_path)
        action_head.load_state_dict(state_dict)
