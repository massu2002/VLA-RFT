...
class RLDSBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.use_minivla:
            ...
            if len(input_ids) >= 3:
                # 删除时从后往前删除，避免索引变化
                del input_ids[-3]  # 倒数第三个数字：220
                del input_ids[-2]  # 倒数第二个数字：151645
                del input_ids[-1]  # 倒数第1个数字：151643
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.use_minivla:
            ...
            if NUM_TOKENS<len(flattened_action_chunk_string):
                ...
                # 从原始数组中随机采样（允许重复）
                extended_array = random.choices(flattened_action_chunk_string, k=remaining_length)  # 随机抽取剩余元素
