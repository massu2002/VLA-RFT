...
class PaddedCollatorForActionPrediction:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        # Stack all raw images if they exist
        if "raw_pixel_values" in instances[0]:
            raw_pixel_values = [torch.from_numpy(np.copy(instance["raw_pixel_values"])) for instance in instances]
            raw_pixel_values = torch.stack(raw_pixel_values)
        else:
